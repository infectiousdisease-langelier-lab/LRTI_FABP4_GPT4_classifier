...
Here I test the various classifiers to achieve 90% sensitivity

```{r}
roc.temp <- pROC::roc(
    cv.folds$LRTI ~ cv.folds$mod2.prob,
    levels=c("0","1"),
    direction="<",
    plot=TRUE, print.auc=TRUE)
print(pROC::coords(
  ...
  x=0.9,
  input="sensitivity",
  ret=c("specificity","threshold")
))
```

I will need to manually calculate the specificity at 90% sensitivity
...
temp <- cv.folds %>%
  select(patient_id, LRTI, mod2.prob) %>%
  mutate(pred=as.factor(mod2.prob>=0.3)) %>%
  group_by(pred, LRTI) %>%
  summarise(count = n(), .groups = 'drop')
print(temp)
```
...
```{r}
ggsave(
  "output/90sensitivity_confusion_matrix.svg",
  plot=p2,

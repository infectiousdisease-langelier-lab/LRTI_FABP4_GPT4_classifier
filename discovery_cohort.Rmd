...
## Import naive physician diagnosis
...
naive$patient_id <- as.character(naive$patient_id)

# Exclude patients 361 and 506 because of QC
naive <- naive %>%
  subset(!(patient_id %in% c("361","506")))
```
...
cv.folds <- metadata.adult %>%
  select(patient_id,
         LRTI, Versa.LRTI.score, Versa.LRTI.score.factor) %>%
  mutate(fold=NA) %>%
  arrange(patient_id)

...
```{r}
counts <- counts[,metadata$StudyID]
stopifnot(colnames(counts) == metadata$StudyID)
```

Plot the total gene counts and number of genes per patient

```{r}
hist(colSums(counts), main="Total gene counts", breaks=10)
```

Number of detected genes

```{r}
hist(colSums(counts>0), main="Number of detected genes", breaks=10)
```
...
# Number of patients below 1e6 counts
print(sum(colSums(counts)<1e6))
```

Filter out patients with below 1e6 counts

```{r}
counts.qc <- counts[,colSums(counts)>=1e6]

hist(colSums(counts.qc>0), main="Number of detected genes after filtering", breaks=10)
```

Filter the metadata file

```{r}
metadata.qc <- metadata[match(colnames(counts.qc),metadata$StudyID),]
stopifnot(colnames(counts.qc)==metadata.qc$StudyID)
```

```{r}
print(table(metadata.qc$LRTI))
```
...
rep0 <- rep(
  1:3, length.out=sum(metadata.qc$LRTI=="0"))
rep1 <- rep(
  1:3, length.out=sum(metadata.qc$LRTI=="1"))

# Assign fold
set.seed(0)
cv.folds <- metadata.qc %>%
...
for (k in c(1:3)) {
  ...
  # Train data
  counts.train <- counts.qc[,train.folds$StudyID]
  ...
  # Test data
  counts.test <- counts.qc[,test.fold$StudyID]
